#pragma once
#include <bits/stdc++.h>
using namespace std;

/**
 * @namespace Basic
 * @brief Fundamental math operations (GCD, LCM, Ceil, Floor).
 * @note STANDALONE: No external dependencies.
 */
namespace Basic {
    using ll = long long;

    /**
     * @brief Ceil Division: Calculates ceil(a / b) safely.
     * @return (a + b - 1) / b, handling overflow logic implicitly.
     */
    template<typename T> 
    inline T ceil_div(T a, T b) { return a / b + ((a % b) != 0); }

    /**
     * @brief Greatest Common Divisor (GCD).
     */
    inline ll gcd(ll a, ll b) { while (b) { a %= b; swap(a, b); } return a; }
    
    /**
     * @brief Least Common Multiple (LCM).
     */
    inline ll lcm(ll a, ll b) { if (a == 0 || b == 0) return 0; return (a / gcd(a, b)) * b; }

    /**
     * @brief Extended Euclidean Algorithm.
     * @return g = gcd(a, b). Updates x, y such that ax + by = g.
     */
    inline ll extGCD(ll a, ll b, ll &x, ll &y) {
        if (b == 0) { x = 1; y = 0; return a; }
        ll x1, y1;
        ll d = extGCD(b, a % b, x1, y1);
        x = y1;
        y = x1 - y1 * (a / b);
        return d;
    }
}